...
class AnimateFromCoeff():
    ...
    def load_cpk_facevid2vid(self, checkpoint_path, generator=None, discriminator=None,
        ...
        if optimizer_kp_detector is not None:
            optimizer_kp_detector.load_state_dict(
                checkpoint['optimizer_kp_detector'])
        if optimizer_he_estimator is not None:
            optimizer_he_estimator.load_state_dict(
    ...
    def generate(self, x, video_save_dir, pic_path, crop_info, enhancer=None, background_enhancer=None, preprocess='crop', img_size=256, pic_path_source='', still_mode=False):
        ...
        if not still_mode:
            ...
            source_semantics_full = x['source_semantics_full'].type(
                torch.FloatTensor)
            target_semantics_full = x['target_semantics_list_full'].type(
                torch.FloatTensor)
        ...
        for idx in range(predictions_video.shape[0]):
            ...
            image = np.transpose(image.data.cpu().numpy(), [
        ...
        if preprocess == "crop" and not still_mode:
            ...
            video_name_nobg = x["video_name"] + "_nobg.mp4"
            full_video_path_nobg = os.path.join(
                video_save_dir, video_name_nobg)
            paste_vid(av_path, av_path_full, crop_info,
                      new_audio_path, full_video_path_nobg, body_h, body_w)
            
            paste_pic(full_video_path_nobg, pic_path_source, crop_info,
