...
print("b")
def click(input_text):
    res = requests.post(
        "http://0.0.0.0:8000/click",
        json={'input': input_text}
        #  data = {'input': input_text}
    )
    if res.status_code == 201 or res.status_code == 308:
        byte = res.content
        return byte.decode('utf-8')  # Assuming the response is text, decode bytes to string
    else:
        return "phanbao"

with gr.Blocks() as demo:
    run_button = gr.Button("Click vo")
