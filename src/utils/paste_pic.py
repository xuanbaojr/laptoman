...
def paste_pic(video_path, pic_path, crop_info, new_audio_path, full_video_path, extended_crop=False):
   ...
   # pic_path = "./results/art_0_360.png"
    ...
    value = 0  # Giá trị này có thể thay đổi tùy ý, tùy vào mức độ giảm sáng bạn muốn
        ...
        for i in range (frame_h):
            for j in range(frame_w - 1):  # Change here to avoid index out of bound
                if test_img[i, j] == 0 :  # Change here to correct the indices
                    crop_frame[i, 0:j+10, :] = np.copy(full_img[i,0:j+10, :])
                    
                    break
            for j in range(frame_w-1, 0, -1):
                if test_img[i, j] == 0 :  # Change here to correct the indices
                    crop_frame[i, j-10:frame_w-1, :] = np.copy(full_img[i, j-10:frame_w-1, :])
                    break
